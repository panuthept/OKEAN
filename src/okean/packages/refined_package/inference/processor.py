...
from okean.packages.refined_package.constants.mapping_constants import wikidata_to_spacy_types
from okean.packages.refined_package.constants.resources_constants import model_name_to_files
from okean.packages.refined_package.data_types.base_types import Entity, Span
from okean.packages.refined_package.data_types.doc_types import Doc
from okean.packages.refined_package.data_types.modelling_types import BatchedElementsTns, ModelReturn
from okean.packages.refined_package.doc_preprocessing.preprocessor import PreprocessorInferenceOnly, Preprocessor
from okean.packages.refined_package.model_components.config import ModelConfig
from okean.packages.refined_package.model_components.refined_model import RefinedModel
from okean.packages.refined_package.resource_management.aws import S3Manager
from okean.packages.refined_package.resource_management.resource_manager import ResourceManager
from okean.packages.refined_package.utilities.general_utils import round_list, get_logger, batch_items
from okean.packages.refined_package.utilities.numeric_handling.date_utils import SpanDateHandler
from okean.packages.refined_package.utilities.preprocessing_utils import convert_doc_to_tensors, convert_batch_elements_to_batched_tns
...
class Refined(object):
    ...
    def process_text(
        ...
        for batch_idx, batch in enumerate(tns):
            batch_spans, inference_times = self.process_tensors(batch=batch, ner_threshold=ner_threshold,
    ...
    def process_tensors(self, batch: BatchedElementsTns, ner_threshold: float = 0.5,
        ...
        ed_targets_predictions_2 = output.candidate_description_scores.argmax(dim=1)
        ed_targets_softmax_2 = output.candidate_description_scores      # output.candidate_description_scores is already softmaxed
        predicted_entity_ids_2 = (
            cand_ids[torch.arange(cand_ids.size(0)), ed_targets_predictions_2].cpu().numpy().tolist()
        )
        predicted_entity_confidence_2 = round_list(
            ed_targets_softmax_2[torch.arange(ed_targets_softmax_2.size(0)), ed_targets_predictions_2]
                .cpu()
                .numpy()
                .tolist(),
            4,
        )
        sorted_entity_ids_scores_2, old_indices = ed_targets_softmax_2.sort(descending=True)
        sorted_entity_ids_scores_2 = sorted_entity_ids_scores_2.cpu().numpy().tolist()
        sorted_entity_ids_2 = self.sort_tensor(cand_ids, old_indices).cpu().numpy().tolist()
        ...
        span_to_sorted_classes = defaultdict(list)
        sorted_pred_class_indices_tns = output.et_activations.argsort(descending=True)
        for span_idx in span_indices.cpu().numpy().tolist():
            sorted_pred_class_indices = [pred_class_idx for pred_class_idx in sorted_pred_class_indices_tns[span_idx].cpu().numpy().tolist() if pred_class_idx != 0]
            class_ids = [self.preprocessor.index_to_class.get(pred_class_idx, "Q0") for pred_class_idx in sorted_pred_class_indices]
            class_labels = [self.preprocessor.class_to_label.get(class_id, "no_label") for class_id in class_ids]
            confs = output.et_activations[span_idx, sorted_pred_class_indices].cpu().numpy().tolist()
            span_to_sorted_classes[span_idx] = list(zip(class_ids, class_labels, confs))
        ...
        for span_idx, span in enumerate(spans):
            ...
            span.top_k_predicted_entities = [
                (
                    Entity(
                        wikidata_entity_id=f'Q{entity_id}',
                        wikipedia_entity_title=self.preprocessor.qcode_to_wiki.get(wikidata_id)
                        if self.preprocessor.qcode_to_wiki is not None else None
                    ),
                    round(score, 8)
                )
                for entity_id, score in zip(sorted_entity_ids[span_idx], sorted_entity_ids_scores[span_idx]) if entity_id != 0
            ]
            span.all_predicted_entities = [
                (
                    Entity(
                        wikidata_entity_id=f'Q{entity_id}',
                        wikipedia_entity_title=self.preprocessor.qcode_to_wiki.get(wikidata_id)
                ...
                for entity_id, score in zip(cand_ids[span_idx].cpu().numpy().tolist(), ed_targets_softmax[span_idx].cpu().numpy().tolist()) if entity_id != 0
            ]
            
            # Entity prediction using entity description scores
            wikidata_id = f'Q{str(predicted_entity_ids_2[span_idx])}'
            span.predicted_entity_2 = Entity(
                wikidata_entity_id=wikidata_id,
                wikipedia_entity_title=self.preprocessor.qcode_to_wiki.get(wikidata_id)
                if self.preprocessor.qcode_to_wiki is not None else None
            )
            span.entity_linking_model_confidence_score_2 = predicted_entity_confidence_2[span_idx]
            span.top_k_predicted_entities_2 = [
                ...
                zip(sorted_entity_ids_2[span_idx], sorted_entity_ids_scores_2[span_idx])
                if entity_id != 0
            ]
            ...
            span.sorted_predicted_entity_types = span_to_sorted_classes[span_idx]

        inference_times = (
            output.encoding_time,
            output.desc_time,
            output.type_time,
            output.ed_time,
        )
        return sorted(spans + predicted_other_spans, key=lambda x: x.start), inference_times
