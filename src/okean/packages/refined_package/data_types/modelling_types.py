...
from okean.packages.refined_package.utilities.general_utils import get_logger, merge_spans, sort_spans
        ...
        for token in self.tokens:
            if token.start in span_indices or token.end - 1 in span_indices:
                ...
                if next_start is not None and token.start >= next_start:
                    next_start = start_indices.pop(0) if len(start_indices) > 0 else None
                    ner_labels.append(ner_tag_to_ix.get("B" + ner_label, 0))
                else:
                    ner_labels.append(ner_tag_to_ix.get("I" + ner_label, 0))
            else:
