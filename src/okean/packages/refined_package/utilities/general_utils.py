...
def get_tokenizer(
    ...
    """
    if transformer_name == "roberta_base_model":
        transformer_path = "roberta-base"
    else:
        transformer_path = "roberta-large"
    if data_dir is not None and os.path.exists(os.path.join(data_dir, transformer_path)):
        tokenizer = AutoTokenizer.from_pretrained(
            os.path.join(data_dir, transformer_path),
            use_fast=use_fast,
