...
class ReFinED(EntityLinking):
    ...
    def __call__(
        ...
        _docs: List[_Doc] = self.refined.process_text_batch(
            ...
            spanss=[[_Span(text=span.surface_form, start=span.start, ln=span.end - span.start) for span in d.entities] for d in passages] if passages[0].relevant_entities is not None else None,
