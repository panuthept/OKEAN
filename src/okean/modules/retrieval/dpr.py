...
class DPR(DenseRetriever):
    ...
    def queries_encoding(self, texts: List[str], batch_size: int = 8) -> np.ndarray:
        all_embeddings = np.zeros((len(texts), self.query_model.config.hidden_size), dtype=np.float32)
        for i in range(0, len(texts), batch_size):
            inputs = self.query_tokenizer(texts[i:i + batch_size], max_length=512, padding=True, truncation=True, return_tensors="pt")
    ...
    def corpus_encoding(self, texts: List[str], batch_size: int = 8) -> np.ndarray:
        ...
        for i in range(0, len(texts), batch_size):
            inputs = self.corpus_tokenizer(texts[i:i + batch_size], max_length=512, padding=True, truncation=True, return_tensors="pt")
            inputs = {k: v.to(self.device) for k, v in inputs.items()}

            with torch.no_grad():
                outputs = self.corpus_model(**inputs)
                embeddings = self._average_pooling(outputs.last_hidden_state, inputs["attention_mask"])
                embeddings = F.normalize(embeddings, p=2, dim=1).detach().cpu().numpy()
                all_embeddings[i:i + batch_size] = embeddings
        return embeddings
...
if __name__ == "__main__":
    retriever = DPR(corpus_path="./corpus/DPR")
